    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            if (isEditing && newItem.id) {
                await api.memos.update(newItem);
            ...
            showToast('error', isEditing ? 'Failed to update memo' : 'Failed to post memo');
        }
    };
                        ...
                        <div key={memo.id} className={`bg-white dark:bg-midnight-900 p-8 rounded-3xl border shadow-sm hover:shadow-md transition-all relative overflow-hidden group ${memo.urgent ? 'border-red-100 dark:border-red-900/30' : 'border-gray-100 dark:border-midnight-800'}`}>
                            ...
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <div className="flex items-center gap-2 text-gray-300">
                                    <button className="hover:text-amber-500 transition-colors"><FiStar size={20} /></button>
                                    {isAdmin && (
                                        <>
                                            <button
            ...
            {isAddOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    <div className="bg-white dark:bg-midnight-900 p-8 rounded-3xl w-full max-w-md shadow-2xl">
                        <h2 className="text-xl font-black mb-6 dark:text-white">{isEditing ? 'Edit Memo' : 'Post Memo'}</h2>
                        <form onSubmit={handleSave} className="space-y-4">
                            ...
                            <div className="flex gap-4 mt-6">
                                ...
                                <button type="submit" className="flex-1 py-3 bg-amber-600 text-white font-bold rounded-xl hover:bg-amber-700">{isEditing ? 'Update' : 'Post'}</button>
                            </div>
                        </form>
                    </div>
                </div>
