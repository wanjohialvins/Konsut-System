    ...
    const loadTasks = useCallback(async () => {
        try {
            ...
            const [data, usersData] = await Promise.all([
                api.tasks.getAll(),
                api.users.getAssignable() // Fetch users for dropdown
            ]);
            ...
            <SmartTableToolbar
                ...
                actions={
                    <button
                        ...
                        className="px-6 py-3 bg-brand-600 hover:bg-brand-700 text-white rounded-2xl font-bold flex items-center gap-2 shadow-lg shadow-brand-900/30 transition-all active:scale-95 text-xs uppercase tracking-widest"
                    >
                        <FiPlus /> New Task
                    </button>
                }
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                {(['admin', 'manager', 'ceo'].includes(user?.role || '')) ? (
                                    ...
                                    <div className="flex items-center gap-2 p-3 bg-gray-100 dark:bg-midnight-950 rounded-xl border border-gray-200 dark:border-midnight-800 text-gray-500">
                                        <FiUser />
                                        <span className="text-sm font-bold">{user?.username} (Self)</span>
                                    </div>
                                )}
                            </div>
                            <div className="flex gap-4 mt-6">
                                <button type="button" onClick={() => setIsAddOpen(false)} className="flex-1 py-3 text-gray-500 font-bold hover:bg-gray-100 dark:hover:bg-midnight-800 rounded-xl transition-colors">Cancel</button>
                                <button type="submit" className="flex-1 py-3 bg-brand-600 text-white font-bold rounded-xl hover:bg-brand-700 shadow-lg shadow-brand-500/30 transition-all active:scale-95">Create Task</button>
                            </div>
                        </form>
                    </div>
                </div>
