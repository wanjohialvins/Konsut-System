...
import { FaFileSignature, FaFileInvoice, FaReceipt, FaSearch, FaSortAmountDown, FaSortAmountUp, FaDollarSign, FaFilePdf, FaCheck, FaExchangeAlt, FaEnvelope, FaShareAlt, FaTrash, FaEye, FaPlus, FaFilter, FaEdit, FaEllipsisV } from "react-icons/fa";
...
const Invoices = () => {
  ...
  const deleteInvoice = useCallback(async (id: string) => {
    ...
    try {
      // setLoading(true); // Don't show full page loader for optimistic action
  ...
  }, [invoices, showConfirm, showToast]);
  ...
  const handleBulkDelete = useCallback(async () => {
    ...
    try {
      ...
      showToast('success', `${idsToDelete.size} documents deleted`);
    } catch {
      // Revert on failure
      setInvoices(previousInvoices);
  ...
  }, [selectedIds, invoices, showConfirm, showToast, loadInvoices]);
                      ...
                      <tr key={inv?.id} className={`hover:bg-blue-50/30 dark:hover:bg-blue-900/10 transition-colors ${isSelected ? "bg-blue-50/50 dark:bg-blue-900/20" : ""}`}>
                        ...
                        <td className="px-6 py-5 text-center px-4 relative">
                          <div className="flex justify-center gap-2">
                            ...
                            <button
                              onClick={(e) => { e.stopPropagation(); previewPDF(inv.id, inv.type, displayCurrency); }}
